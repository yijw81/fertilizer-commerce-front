...
import type { ProductOption, Review } from '../types/fertilizer'
...
const buyNow = () => {
  store.addToCart(productId.value, selectedOption.value ?? undefined)
  ...
  <section v-if="product" class="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
    ...
    <div class="space-y-6 rounded-[2rem] bg-white p-8 shadow-sm">
      ...
      <select v-model="selectedOption" class="input">
        <option v-for="opt in product.options" :key="opt.label" :value="opt">
          {{ opt.label }}
      ...
      <div v-if="selectedOption" class="flex items-end gap-3">
        <strong class="text-3xl">
          {{ formatPrice(product.price + selectedOption.priceAdjustment) }}원
        </strong>
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        ...
        <button class="secondary-btn" @click="store.addToCart(product.id, selectedOption ?? undefined)">장바구니 담기</button>
      </div>
