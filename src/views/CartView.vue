...
<template>
  <section class="grid gap-8 lg:grid-cols-[1fr_360px]">
    <div class="space-y-4">
      ...
      <article v-for="item in store.cartDetails" :key="`${item.productId}-${item.option}`" class="flex gap-4 rounded-3xl bg-white p-4 shadow-sm">
        ...
        <div class="flex-1">
          ...
          <div class="mt-3 flex items-center gap-2">
            <button class="qty-btn" @click="store.updateQuantity(item.productId, item.option, item.quantity - 1)">-</button>
